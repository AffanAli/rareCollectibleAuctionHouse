...
**Rare Collectible Auction House** — coursework backend (NestJS on **Express** via `@nestjs/platform-express`). Controllers expose REST-shaped routes; services hold stub logic until persistence and auth are added. This is a structured **module/controller/service** layout (similar to MVC routing + logic layers), not a production deployment.
...
Frontend (e.g. React) can call this API with **CORS** enabled in `main.ts`. Copy `.env.example` to `.env` if you set `PORT` or `CORS_ORIGIN`.
